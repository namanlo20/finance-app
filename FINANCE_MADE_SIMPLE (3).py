...
# Using v3 endpoints (v4 is for legacy users only)
...
def get_quote(ticker):
    """Get real-time stock quote - Updated endpoint"""
    # Try the stock-price-quote endpoint first (modern)
    url = f"https://financialmodelingprep.com/api/v4/quote/{ticker}?apikey={FMP_API_KEY}"
    try:
        response = requests.get(url, timeout=10)
