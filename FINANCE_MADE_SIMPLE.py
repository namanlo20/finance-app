...
# ================== TAB SUMMARY POPUPS (NON-REDIRECT, AUTO-HIDE) ==================
# These popups:
# - Never change the current page
# - Can be dismissed via X
# - Auto-hide after ~10 seconds (client-side JS; no rerun/redirect)
import re as _tabpop_re

def _tabpop_slugify_page(_p: str) -> str:
    _p = _p or ""
    _p = _tabpop_re.sub(r"\s+", "_", _p.strip())
    _p = _tabpop_re.sub(r"[^a-zA-Z0-9_]", "", _p)
    return _p[:60] if _p else "page"
...
TAB_POPUP_COPY = {
    "📖 Basics": {
        ...
        "bullets": [
            ...
            "Built to keep momentum: simple, clear, and practical.",
        ],
    },
    "📚 Finance 101": {
        "title": "Finance 101 — level up the fundamentals",
        "bullets": [
            "Understand statements + cash flow without the textbook headache.",
            "See the metrics that actually move stocks, explained simply.",
            "Perfect warm-up before Pro/Ultimate workflows.",
        ],
    },
    "🧠 Risk Quiz": {
        ...
        "bullets": [
            ...
            "Use your result to guide what you research and how you size positions.",
            "Less overwhelm, more clarity — instantly.",
        ],
    },
    "📊 Company Analysis": {
        "title": "Company Analysis — go from ‘ticker’ to conviction",
        "bullets": [
            "One clean view of fundamentals: growth, margins, valuation, durability.",
            "Fast ‘what’s strong / what’s weak / what to watch’ structure.",
    ...
    "📊 Market Overview": {
        ...
        "bullets": [
            "Scan what’s moving across the market without drowning in noise.",
            "Spot rotation early so you know what to research next.",
            "Best used as a quick daily 2–3 minute check-in.",
        ],
    },
    "🔍 AI Stock Screener": {
        "title": "AI Stock Screener — find ideas without scrolling forever",
        "bullets": [
            "Filter the universe down to a focused shortlist in seconds.",
            "Start broad, then narrow — defaults always show results.",
            "Click straight into deep dives when something looks interesting.",
        ],
    },
    "📈 Financial Health": {
        "title": "Financial Health — stress-test a company quickly",
        "bullets": [
            "Catch leverage/liquidity red flags early — before you fall in love with a chart.",
            "Quick durability checks built for real-world decision-making.",
            "Great for avoiding ‘looks good until it doesn’t’ names.",
        ],
    },
    "📰 Market Intelligence": {
        "title": "Market Intelligence — context that makes charts make sense",
        "bullets": [
            "High-signal market context so you’re not trading blind.",
            "Connect headlines to ‘what it could mean’ without doomscrolling.",
    ...
    "💼 Paper Portfolio": {
        ...
        "bullets": [
            ...
            "See holdings, cost basis, P/L, and trade history — like a real broker view.",
            "Best way to build discipline before using real money.",
        ],
    },
    "👤 Naman's Portfolio": {
        "title": "Naman’s Portfolio — founder picks in one view",
        "bullets": [
            "See the founder paper portfolio positioning and track it over time.",
            "Use it for study + ideas (educational, not advice).",
            "Compare against your own watchlist and learn faster.",
        ],
    },
    "📜 Founder Track Record": {
        "title": "Founder Track Record — full transparency, no cherry-picking",
        "bullets": [
            "Immutable trade ledger so the history stays honest.",
            "Holdings snapshot + performance summary in one clean page.",
            "Built to earn trust: every move stays visible.",
        ],
    },
    "👑 Become a VIP": {
        "title": "VIP — unlock speed + depth upgrades",
        "bullets": [
            "See what VIP adds: faster workflows, deeper tools, and premium utilities.",
            "Core learning stays accessible — VIP is acceleration, not a paywall.",
            "Upgrade when you want to move faster and go deeper.",
        ],
    },
    "📊 Pro Checklist": {
        "title": "PRO — your repeatable investing system (step-by-step)",
        "bullets": [
            "A structured checklist that turns research into a repeatable workflow.",
            "Know exactly what to evaluate (and in what order) so you stop second-guessing.",
            "Built for consistency: same process for every ticker, every time.",
            "Ideal bridge from ‘learning’ to ‘execution’ — without chaos.",
        ],
    },
    "👑 Ultimate": {
        ...
        "bullets": [
            ...
            "Deep analysis prompts + advanced checks that surface risks and catalysts.",
    ...
    "✅ Portfolio Risk Analyzer": {
        ...
        "bullets": [
            "See concentration, exposure, and risk balance across holdings.",
            "Catch ‘too much in one theme’ before the market teaches you the hard way.",
            "Turn positions into a portfolio with intention.",
        ],
    },
}

def render_tab_summary_popup(current_page: str) -> None:
    # Welcome popup is handled inside Start Here (and should never stack here).
    if current_page == "🏠 Start Here":
        return

    copy = TAB_POPUP_COPY.get(current_page)
    if not copy:
        return

    seen_key = f"_tab_popup_seen::{current_page}"
    if st.session_state.get(seen_key, False):
        return

    # Mark seen immediately so it won't re-render on reruns.
    st.session_state[seen_key] = True

    popup_id = _tabpop_slugify_page(current_page)
    title = copy["title"]
    ...
    <style>
      ...
      .tabpop_card_{popup_id} {{
        width: min(780px, 92vw);
        background: linear-gradient(180deg, rgba(14,19,36,0.98), rgba(9,12,24,0.98));
        border: 2px solid rgba(0,217,255,0.65);
        border-radius: 18px;
        padding: 28px 26px 22px 26px;
        box-shadow: 0 20px 60px rgba(0,0,0,0.55);
        position: relative;
      }}
      .tabpop_title_{popup_id} {{
        font-size: 34px;
        font-weight: 800;
        line-height: 1.15;
        margin: 0 0 14px 0;
    ...
    <div class="tabpop_overlay_{popup_id}" id="tabpop_overlay_{popup_id}">
      <div class="tabpop_card_{popup_id}">
        <button class="tabpop_close_{popup_id}" id="tabpop_close_{popup_id}" aria-label="Close">×</button>
        <div class="tabpop_title_{popup_id}">{title}</div>
        <div class="tabpop_sub_{popup_id}">Quick summary — auto-closes in ~10 seconds.</div>
        <ul class="tabpop_ul_{popup_id}">
          {bullets_html}
        </ul>
        <div class="tabpop_tip_{popup_id}">Tip: this never changes your page — it’s just a quick orientation.</div>
      </div>
    </div>

    <script>
      (function() {{
        const overlay = document.getElementById("tabpop_overlay_{popup_id}");
        const closeBtn = document.getElementById("tabpop_close_{popup_id}");
...
# Show the per-tab summary popup (non-redirect, auto-hides)
render_tab_summary_popup(selected_page)
...
# This replaces lines 7343-8025 in the main file
    ...
    LEARN_HUB_LESSONS = {
        ...
        "B1": {
            ...
            "summary": [
                "Buying stock represents ownership in a company",
            ...
            "common_mistakes": [
                "Thinking stock price = company quality",
            ...
            "quiz": [
                {
                    "question": "Buying stock represents:",
                    "options": ["A) Ownership", "B) Loan to company", "C) Coupon for products", "D) Guarantee of profits"],
                    "correct": 0,
                    "explanation": "Stocks give you fractional ownership in a company, including claims on future profits."
                },
                {
                    "question": "Prices move mainly due to:",
                    "options": ["A) Dividends daily", "B) Expectations", "C) CEOs set them", "D) Illegal activity"],
                    "correct": 1,
                    "explanation": "Stock prices primarily move based on changing expectations about future performance."
                },
                {
        ...
        "B2": {
            ...
            "summary": [
                "Compounding means returns on returns",
            ...
            "checklist": [
                "Set realistic time horizon (5+ years)",
            ...
            "quiz": [
                ...
                },
                {
                    ...
                    "options": ["A) Eliminating risk", "B) Letting you ride cycles", "C) Can't drop", "D) Guaranteed wins"],
                    "correct": 1,
        ...
        "B3": {
            ...
            "key_ideas": [
                "Index reduces single-company risk",
            ...
            "quiz": [
                {
                    "question": "Index fund reduces:",
                    "options": ["A) Interest rates", "B) Single-company risk", "C) Inflation", "D) Foreign exchange risk"],
                    "correct": 1,
                    "explanation": "Index funds spread risk across many companies, reducing single-company risk."
                },
                {
                    "question": "Core-satellite means:",
                    "options": ["A) Cash + bonds", "B) Index core + small stock bets", "C) Options only", "D) One sector only"],
                    "correct": 1,
                    "explanation": "Core-satellite = stable index foundation + smaller individual stock positions."
                },
                {
                    "question": "Loss aversion means:",
                    "options": ["A) Love taking risks", "B) Losses hurt more than gains feel good", "C) Always buy dips", "D) Dividends only"],
                    "correct": 1,
                    "explanation": "Loss aversion = psychological tendency where losses feel twice as bad as equal gains feel good."
                }
            ],
        ...
        "B4": {
            ...
            "summary": [
                "ETF holds a basket of stocks in one ticker",
            ...
            "quiz": [
                ...
                },
                {
                    ...
                    "options": ["A) Match benchmark performance", "B) Password tracking", "C) Earnings tracking", "D) Rate tracking"],
                    "correct": 0,
        ...
        "B5": {
            ...
            "summary": [
                "Diversification reduces single error impact",
            ...
            "key_ideas": [
                "True diversification = different drivers",
            ...
            "common_mistakes": [
                "Thinking more stocks = more diversified",
            ...
            "quiz": [
                {
                    ...
                    "options": ["A) Adds excitement", "B) Reduces single error impact", "C) Guarantees profits", "D) Removes volatility"],
                    "correct": 1,
                    "explanation": "Diversification reduces the impact of any single investment mistake or company-specific risk."
                },
                {
                ...
                },
                {
                    ...
                    "correct": 1,
                    "explanation": "Idiosyncratic risk is specific to individual companies and can be reduced through diversification."
                }
            ],
        ...
        "I2": {
            ...
            "summary": [
                "Durable growth = repeat demand + pricing power",
            ...
            "key_ideas": [
                "Check if growth is organic vs M&A",
            ...
            "common_mistakes": [
                "Chasing growth without checking quality",
            ...
            "quiz": [
                {
                    "question": "Durable growth supported by:",
                    "options": ["A) One-time events", "B) Repeat demand + pricing", "C) Dilution", "D) Debt"],
                    "correct": 1,
                    "explanation": "Sustainable growth comes from repeatable demand and ability to maintain/raise prices."
                },
                {
                    "question": "Hype risk:",
                    "options": ["A) Too much cash", "B) Expectations > reality", "C) Too many customers", "D) Low volatility"],
                    "correct": 1,
        ...
        "A1": {
            ...
            "summary": [
                "Hidden correlation = different names, same drivers",
            ...
            "key_ideas": [
                "Correlations change during crashes",
            ...
            "common_mistakes": [
                "Assuming different tickers = diversified",
            ...
            "checklist": [
                "List key driver for each holding",
            ...
            "quiz": [
                ...
                },
                {
                    ...
                    "correct": 1,
                    "explanation": "True diversification requires different fundamental drivers, not just different names."
                }
            ],
        ...
        "P1": {
            ...
            "summary": [
                "Loss aversion = losses hurt ~2x more than equal gains",
                "Causes panic selling at worst times",
                "Defense = pre-committed process",
                "Rules beat emotions"
            ],
            "key_ideas": [
                "Biological bias, not weakness",
            ...
            "checklist": [
                "Set rules before entering positions",
            ...
            "quiz": [
                {
                    "question": "Loss aversion:",
                    "options": ["A) Love volatility", "B) Losses hurt more than gains feel good", "C) Gains worse", "D) ETFs only"],
                    "correct": 1,
                    "explanation": "Loss aversion = psychological bias where losses feel ~2x worse than equivalent gains."
                },
                {
                    "question": "Causes:",
        ...
        "R1": {
            ...
            "quiz": [
                {
                    "question": "Weakness sign:",
                    "options": ["A) Stable revenue", "B) Multi-year declining revenue + margins", "C) Strong FCF", "D) Low debt"],
                    "correct": 1,
                    "explanation": "Sustained revenue and margin decline signals fundamental business deterioration."
                },
                {
                    "question": "Debt danger:",
                    "options": ["A) Cash flow strong", "B) Cash flow weak + debt grows", "C) Volatility rises", "D) Dividends grow"],
                    "correct": 1,
